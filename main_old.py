...
for I in range(150):
    results = play_games(lambda: TicTacToeGame(), [dqn_first, human],1,paths= [model1,model2], plot=True, debug=True)
    print("kolejne epoki: " + str(I) + "   ---->")
    plot_game_results(results, 2,100,[model1,model2], "(offen awarded) " + str(I))
    dqn_first.model.summary()
    dqn_second.model.summary()
