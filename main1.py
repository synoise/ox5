...
from game.agents.ab_runing import ABPruning
...
def initiateAgents():
    ...
    # dqn_second1 = DoubleDQNAgentEndMatrixEnd(i_agent=0,
    #                                   is_learning=False,
    #                                   learning_rate=0.001,
    #                                   gamma=0.99,
    #                                   epsilon=1.0,
    #                                   epsilon_end=0.001,
    #                                   epsilon_decay_linear=1 / 5000,
...
for I in range(150):
    ...
    results = play_games(lambda: TicTacToeGame(), [dqn_first, dqn_second], 1500, paths = [first1, "non"], plot=False, debug=True)
    print("kolejne epoki: " + str(I) + "   ----> seed1:"+ str(seed1) +" -  seed2:"+ str(seed2))
    # plot_game_results(results, 2, 100, [first1, second2], " _ " + str(I))
    stats.saveStats('./stats/fixed_DDQN_10N10x3_Stats.json', seed1, first1, seed2, "noni", results, 1500)
    first1 = 'FIX_MATRIX_first_08.06_progres.h5'
    path_first1 = folder + first1
